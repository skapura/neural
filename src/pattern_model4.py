...
class BinaryToCategorical(layers.Layer):
    ...
    def call(self, inputs):
        ...
        sz = 0 if inputs.shape[0] is None else inputs.shape[0]
        m = tf.reshape(m, shape=(sz, self.num_classes))
        return m
...
class PatternLayer(layers.Layer):
    ...
    def match_predict(self, matches, feat_outs, model, prediction_buffer):
        if matches.shape[0] is not None and matches.shape[0] > 0:
            batch = tf.gather(feat_outs, indices=matches, axis=0)
            preds = model(batch)
            sz = 0 if matches.shape[0] is None else matches.shape[0]
            for i in range(sz):
                prediction_buffer = prediction_buffer.write(matches[i], preds[i])
        return prediction_buffer

    def fit(self, ds, **kwargs):

        #totalmatches = tf.TensorArray(tf.int64, size=0, dynamic_size=True)
        totalmatches = list()
        for step, (x_batch, y_batch) in enumerate(ds):
            print(step)
            feats = self.feat_extract(x_batch)
            ...
            print(len(x_batch))
            if len(matches) > 0:
                lm = matches.numpy().tolist()
                lm = [l + step * len(x_batch) for l in lm]
                totalmatches += lm
                #totalmatches.write(step, matches)
            #print(matches)
        print(1)
        return totalmatches
    ...
    def call(self, inputs):
        ...
        sz = 0 if inputs.shape[0] is None else inputs.shape[0]
        mergedpreds = tf.TensorArray(inputs.dtype, size=sz)

        mergedpreds = self.match_predict(matches, pouts[0], self.pat_pred, mergedpreds)
        mergedpreds = self.match_predict(nonmatches, feats, self.base_pred, mergedpreds)

        preds = mergedpreds.stack()
        return preds

class PatModel(keras.Model):
    ...
    def fit(self, ds, **kwargs):
        ...
        return self.pat_layer.fit(ds, **kwargs)
