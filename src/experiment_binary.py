...
def run():
    ...
    #trans.to_csv('session/trans_golf.csv')
    ...
    #matches = trans.loc[trans['label'] == 2]
    #matches = trans.loc[trans['path'] == 'datasets/images_large/train/golf_ball/n03445777_16067.JPEG']
    #for index, _ in matches.iloc[:35].iterrows():
    #    print(index)
    #    filters = patmodel.filters_in_layer('activation')
    #    path = data.image_path(trans, index)
    #    prefix = str(index).zfill(4)
    #    plot.output_features(patmodel, path, filters, 'session/images_full/' + prefix, franges)

    # Restrict patterns to layer
    #col = bdf.columns.difference(const.META, sort=False)
    col = [c for c in bdf.columns if 'activation_2-' in c]

    sel = bdf.loc[bdf['label'] == 1.0].drop(const.META, axis=1)[col]
    notsel = bdf.loc[bdf['label'] != 1.0].drop(const.META, axis=1)[col]
    minsup = 0.1
    minsupratio = 1.1
